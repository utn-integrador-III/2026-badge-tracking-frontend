...
export function ShareQr({ proof, ttlSeconds }: ShareQrProps) {
  ...
  return (
    <section className="flex w-full flex-col items-center gap-5" aria-label="Código QR temporal">
      <div className="relative rounded-3xl bg-white p-5 shadow-xl ring-1 ring-slate-200">
        <QRCodeSVG value={token.payload} size={232} level="M" includeMargin aria-label="QR de verificación" />
        {expired ? (
          <div className="absolute inset-0 grid place-items-center rounded-3xl bg-white/95 p-6">
            <div className="text-center">
              ...
              <p className="mt-3 font-bold text-slate-900">Este QR expiró</p>
      ...
      <div className="w-full rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-left text-sm text-emerald-900">
        <p className="flex items-center gap-2 font-semibold"><ShieldCheck className="h-4 w-4" aria-hidden /> Información incluida</p>
        <p className="mt-2">Nombre, rol, número de identificación, institución, estado y vigencia.</p>
      </div>
